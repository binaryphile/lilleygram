...
func NewCertificateController(repo sql.CertificateRepo, specific map[string][]Middleware, general ...Middleware) Controller {
	c := Controller{
		"add":  addCertificate(repo),
		"list": listCertificates(repo),
	}
...
func addCertificate(repo sql.CertificateRepo) HandlerFunc {
...
func listCertificates(repo sql.CertificateRepo) HandlerFunc {
	return func(writer ResponseWriter, request *Request) {
		user, _ := UserFromContext(request.Context)
